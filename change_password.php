...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password === $confirm_password) {
        ...
        if ($result->num_rows > 0) {
            ...
            $update_query = "UPDATE karyawan SET password = ? WHERE id_card = ?";
            $stmt = $koneksi->prepare($update_query);
            $stmt->bind_param("ss", $hashed_password, $id_card);
            $stmt->execute();
            ...
            $stmt->bind_param("s", $token);
            $stmt->execute();
            echo "<script>alert('Password berhasil diubah.'); window.location.href='signin.php';</script>";
        } else {
            // Hapus token yang kedaluwarsa (lebih dari 30 menit)
            $delete_expired_query = "DELETE FROM password_resets WHERE token = ? AND expires_at < NOW()";
            $stmt = $koneksi->prepare($delete_expired_query);
            $stmt->bind_param("s", $token);
            $stmt->execute();
            echo "<script>alert('Token tidak valid atau sudah kedaluwarsa.'); window.location.href='reset-password.php';</script>";
        }
    } else {
        echo "<script>alert('Password tidak cocok.'); window.location.href='change_password.php';</script>";
    }
}
?>
...
<head>
    ...
    <?php
    include 'navbar.php'; ?>
    <title>SISKA RFID - CV Mulia Abadi</title>
</head>
                                ...
                                <div class="auth-form">
                                    ...
                                    <form action="" method="post">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="password" class="form-control" name="confirm_password" id="confirm_password" placeholder="Tuliskan Konfirmasi Password Anda" required>
                                        </div>
