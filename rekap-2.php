                                ...
                                <div class="table-responsive">
                                    <table id="example4" class="display" style="min-width: 845px">
                                        <div class="d-flex justify-content-between mb-3">
                                            <!-- Form Filter -->
                                            <?php
                                            // Tangkap nilai yang dikirim melalui form filter, atau gunakan default bulan dan tahun saat ini
                                            ...
                                            <form method="GET" action="">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <select name="tahun" id="tahun" class="form-select me-2">
                                                        <?php
                                        ...
                                        <div class="justify-content-between mb-3">
                                            <!-- Live Filter berdasarkan hari -->
                                            <button class="btn btn-dark light me-3" onclick="printTable()"><i class="las la-print me-3 scale5"></i>Cetak (PDF)</button>
                                            <button class="btn btn-success me-3" onclick="exportToExcel()"><i class="fas fa-file-excel me-3 scale5"></i>Export to Spreadsheet</button>
                                            <script>
                                                function exportToExcel() {
                                                    let table = document.getElementById('example4');
                                                    let html = table.outerHTML;
                                                    let url = 'data:application/vnd.ms-excel,' + escape(html);
                                                    let link = document.createElement('a');
                                                    link.href = url;
                                                    let date = new Date();
                                                    let dateString = date.toLocaleDateString('en-GB').replace(/\//g, '-');
                                                    link.setAttribute('download', 'rekapitulasi-' + dateString + '.xls');
                                                    link.click();
                                                }
                                            </script>
                                            <script>
                                        ...
                                        <tbody>
                                            ...
                                            // Ambil filter bulan
                                            // Ambil bulan dan tahun dari form filter, gunakan nilai default jika tidak ada input
                                            $filter_bulan = isset($_GET['bulan']) ? $_GET['bulan'] : date('m');
                                            $filter_tahun = isset($_GET['tahun']) ? $_GET['tahun'] : date('Y');
                                            ...
                                            $sql = mysqli_query($koneksi, "SELECT 
                                                    k.id_card, 
                                                    k.nama, 
                                                    COUNT(CASE WHEN p.status = 'Tepat Waktu' THEN 1 END) AS total_masuk, 
                                                    COUNT(CASE WHEN p.status_pulang = 'Tepat Waktu' THEN 1 END) AS total_pulang, 
                                                    COUNT(CASE WHEN p.status = 'Terlambat' THEN 1 END) AS total_terlambat,
                                                    COUNT(CASE WHEN p.status_pulang = 'Belum Presensi' THEN 1 END) AS total_belum_presensi,
                                                    (COUNT(CASE WHEN p.status = 'Tepat Waktu' THEN 1 END) + COUNT(CASE WHEN p.status_pulang = 'Tepat Waktu' THEN 1 END)) AS total
                                                FROM 
                                                    karyawan k
                                                LEFT JOIN 
                                                    presensi p
                                                ON k.id_card = p.id_card
                                                WHERE 
                                                    MONTH(p.tanggal) = '$filter_bulan' AND YEAR(p.tanggal) = '$filter_tahun'
                                                GROUP BY 
                                                    k.id_card, k.nama
                                                ORDER BY 
                                                    k.nama ASC
                                            ...
                                            ?>
                                                <tr>
                                                    <td><?php echo $no; ?></td>
                                                    <?php
                                                    $id_karyawan = mysqli_query($koneksi, "SELECT id FROM karyawan WHERE id_card = '{$data['id_card']}'");
                                                    $id_karyawan = mysqli_fetch_assoc($id_karyawan);
                                                    ?>
                                                    <td><a href="profile_karyawan.php?id=<?php echo $id_karyawan['id']; ?>"><?php echo $data['id_card'] ?></a></td>
                                                    <td><?php echo $data['nama'] ?></td>
                                                    <td><?php echo $data['total_masuk'] ?></td>
                                                    <td><?php echo $data['total_pulang'] ?></td>
                                                    <td class="text-danger"><?php echo $data['total_terlambat'] ?></td>
                                                    <td class="text-danger"><?php echo $data['total_belum_presensi'] ?></td>
