...
$sql = "SELECT waktu_masuk_start, waktu_masuk_end, waktu_pulang_start, waktu_pulang_end FROM operasional WHERE id=1";
$result = $koneksi->query($sql);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $waktu_masuk_start = $row['waktu_masuk_start'];
    $waktu_masuk_end = $row['waktu_masuk_end'];
    $waktu_pulang_start = $row['waktu_pulang_start'];
    $waktu_pulang_end = $row['waktu_pulang_end'];
} else {
    // Jika data belum ada, beri nilai default
    $waktu_masuk_start = '';
    $waktu_masuk_end = '';
    $waktu_pulang_start = '';
    $waktu_pulang_end = '';
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title ml-2">Jam Operasioanal Kerja</h4>
                    </div>
                    <div class="card-body">
                        <div class="basic-form">
                            <form method="POST" action="set_operasional.php" class="form-inline d-flex justify-content-between gap-3 flex-wrap">
                                <div class="row align-items-center p-3">
                                    <div class="col-sm-10">
                                        <div class="mb-3 row">
                                            <label for="waktu_masuk_start">Waktu Masuk Awal:</label>
                                            <input type="time" id="waktu_masuk_start" class="form-control mb-2 ml-2" name="waktu_masuk_start" value="<?php echo $waktu_masuk_start; ?>" required>
                                        </div>
                                        <div class="mb-3 row">
                                            <label for="waktu_masuk_end">Waktu Masuk Akhir:</label>
                                            <input type="time" id="waktu_masuk_end" class="form-control mb-2 ml-2" name="waktu_masuk_end" value="<?php echo $waktu_masuk_end; ?>" required>
                                        </div>
                                        <div class="mb-3 row">
                                            <label for="waktu_pulang_start">Waktu Pulang Awal:</label>
                                            <input type="time" id="waktu_pulang_start" class="form-control mb-2 ml-2" name="waktu_pulang_start" value="<?php echo $waktu_pulang_start; ?>" required>
                                        </div>
                                        <div class="mb-3 row">
                                            <label for="waktu_pulang_end">Waktu Pulang Akhir:</label>
                                            <input type="time" id="waktu_pulang_end" class="form-control mb-2 ml-2" name="waktu_pulang_end" value="<?php echo $waktu_pulang_end; ?>" required>
                                        </div>
                                        <div class="col-12">
