...
if (isset($_POST['btnSubmit'])) {
    ...
    $simpan = mysqli_query($koneksi, "INSERT INTO karyawan (id_card, nama, password, job, gender, level) VALUES ('$id_card', '$nama', '$password', '$job', '$gender','$level')");
    if ($simpan) {
        echo "<script>
        alert('Data berhasil disimpan');
        location.replace('add_karyawan.php');
        </script>";
    } else {
        echo "<script>
...
<head>
    ...
    <!-- Read Kartu RFID Otomatis -->
    <script type="text/javascript">
        $(document).ready(function() {
            setInterval(function() {
                $('#norfid').load('read_idcard.php')
            }, 0); //refresh no kartu rfid setiap 1 detik
        });
    </script>
</head>
                            ...
                            <div class="basic-form">
                                <form method="POST">
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="level" id="level" class="form-select" required readonly>
                                                <option value="karyawan">User / Karyawan</option>
                                            </select>
                                            <input type="hidden" name="level" id="level" value="karyawan">
                                        </div>
                                    </div>
                                    <fieldset class="mb-3">
                                        <div class="row">
